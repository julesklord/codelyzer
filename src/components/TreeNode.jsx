...
export const TreeNode = React.memo(function TreeNodeInner(props){
    ...
    return React.createElement('div',{role:'group'},
        React.createElement('div',{className:'tree-folder'+(isFiltered?' filtered':''),'aria-selected':isFiltered?true:undefined,tabIndex:0,role:'treeitem','aria-expanded':isOpen,onClick:function(){if(node.path==='')filterFolder(null);else filterFolder(node.path);},onKeyDown:function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();if(node.path==='')filterFolder(null);else filterFolder(node.path);}}},
            React.createElement('span',{className:'tree-toggle'+(isOpen?' open':''),tabIndex:0,role:'button','aria-label':isOpen?'Collapse folder':'Expand folder',title:isOpen?'Collapse folder':'Expand folder','aria-expanded':isOpen,onClick:function(e){e.stopPropagation();toggle(node.path);},onKeyDown:function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();toggle(node.path);}}},children.length>0||node.files.length>0?'▶':''),
        ...
        isOpen&&React.createElement('div',{className:'tree-children'},
            ...
            node.files.map(function(f){
                var isSelected = selected&&selected.path===f.path;
                return React.createElement('div',{key:f.path,className:'tree-file'+(isSelected?' active':''),'aria-selected':isSelected?true:undefined,tabIndex:0,role:'treeitem',onClick:function(){onSelect(f.path);},onKeyDown:function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(f.path);}}},React.createElement(Icon,{name:'file',size:'s',className:'tree-entry-icon'}),React.createElement('span',{className:'tree-name'},f.name));})
        )
    );
}, areEqual);
